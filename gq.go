...
// Return a chan []byte that carries the output from the queue. If the queue doesn't exist,
// create it and set up a receiver
// func GetQueue(name string) (c chan []byte) {
// 	if svc == nil {
// 		// initService()
// 	}
// 	queues.RLock()
// 	if q, exists := queues.m[name]; !exists {
// 		queues.RUnlock()
// 		queues.Lock()
...
func receiveQueue(name string) {
	// for {
	// 	msg, err := svc.ReceiveOne(name)
	// 	if err != nil {
	// 		log.Println(err)
	// 	} else if msg != nil {
	// 		queues(name) <- []byte(msg.Body)
